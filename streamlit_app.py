...
def _auth_login(col):
    ...
    if st.button("Log In", width=True, type="primary"):
...
def _auth_register(col):
    ...
    if st.button("Sign Up", width=True, type="primary"):
    ...
    if st.button("Back to Login", width=True):
...
def _auth_verify(col):
    ...
    with c1:
        if st.button("Verify", width=True, type="primary"):
    ...
    with c2:
        # Fixed: Changed width="stretch" to width=True
        if st.button("Resend Code", width=True):
...
def page_browse():
    ...
    with c_s1:
        search_clicked = st.button("Search", width=True)
    with c_s2:
        show_all = st.button("Show All", width=True)
    if show_all:
...
def _browse_edit_form(row: dict, is_duplicate: bool = False):
    ...
    with st.form(key=f"edit_form_{fn}_{is_duplicate}"):
        ...
        if st.form_submit_button("Save Changes", width=True):
            db_update(fn, {
...
def _browse_duplicate_form(source_row: dict):
    ...
    with st.form(key=f"dup_form_{source_row['FileNumber']}"):
        ...
        if st.form_submit_button("Save as New Record", width=True):
            fn = int(fn)
            _conn = get_pg_conn()
            _cur  = _conn.cursor()
...
def page_add():
    ...
    with st.form("add_form", clear_on_submit=True):
        ...
        submitted = st.form_submit_button("Save Seed", width=True)
...
def page_remove():
    ...
    with c_r1:
        if st.button("Search", width=True):
            st.session_state.remove_term = term
    with c_r2:
        if st.button("Show All", width=True):
...
def page_labels():
    ...
    with sa:
        if st.button("Search", width=True, key="lbl_srch"):
            st.session_state.label_term      = term
            st.session_state.label_pdf_bytes = None
            st.rerun()
    with sb:
        if st.button("Load All", width=True, key="lbl_all"):
            st.session_state.label_term      = ""
            st.session_state.label_pdf_bytes = None
            st.rerun()
    with sc:
        if st.button("Set All to 1", width=True, key="lbl_set1"):
            for r in rows:
                st.session_state.label_qtys[int(r["FileNumber"])] = 1
            st.session_state.label_pdf_bytes = None
            st.rerun()
    with sd:
        if st.button("Clear All", width=True, key="lbl_clr"):
    ...
    with tab2:
        ...
        if sel_user:
            ...
            if sel_user["is_approved"]:
                if c1.button("Revoke Access", width=True):
                    admin_revoke(sel_user["id"])
                    st.rerun()
            else:
                if c1.button("Approve", width=True,
                             type="primary"):
            ...
            new_role = c2.selectbox(
                ...
                key="role_sel")
            if c2.button("Set Role", width=True):
                conn = get_pg_conn()
                cur  = conn.cursor()
                cur.execute(
                ...
                st.rerun()
            if c3.button("Delete User", width=True):
